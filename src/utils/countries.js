// Liste exhaustive des pays/territoires ISO 3166-1 avec nom français.
// Tri alphabétique. Le code (alpha-2) sert pour les API (ex zippopotam.us)
// et l'affichage type "FR" en regard du champ.
...
const NAME_TO_CODE = COUNTRIES.reduce((acc, c) => {
  acc[c.name.toLowerCase()] = c.code;
  return acc;
}, {});

/** Retourne le code ISO depuis le nom français (ou null si inconnu). */
export function countryNameToCode(name) {
  if (!name) return null;
  return NAME_TO_CODE[String(name).trim().toLowerCase()] || null;
}

/** Retourne le pays par son code ISO (ou null). */
